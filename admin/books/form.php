...
elseif($i==3)
{
	...
	echo "<input type='hidden' name='$name' value='$_SESSION[uid]' class='form-control' >";
}
			  ...
			  <label>".str_replace('_', ' ', $name)."</label>";
               ...
               echo "<select name='$name' class='form-control'>";
				...
				while($row2 =mysqli_fetch_array($result2))
				{
					...
					echo "<option value='$row2[id]'>$row2[class]</option>";
				}
	          echo "</select>";
	          echo "</div>
              </div>";
			  ...
			  <label>".str_replace('_', ' ', $name)."</label>";
               ...
               echo "<select name='$name' class='form-control'>";
				...
				while($row2 =mysqli_fetch_array($result2))
				{
					...
					echo "<option value='$row2[id]'>$row2[semester]</option>";
				}
	          echo "</select>";
	          echo "</div>
              </div>";
 ...
 elseif($i==8 )
  {
	  echo "<div class='col-md-10'>
				...
				<label>".str_replace('_', ' ', $name)."</label>";
				echo "<select name='$name' class='form-control'>";
				echo "<option value='active'>free</option>";
				echo "<option value='deactive'>paid</option>";
				echo "</select>";
	  echo "</div>
            </div>";
 ...
 elseif($i==70)
  {
	  echo "<div class='col-md-10'>
              <div class='form-group'>
			  <label>".str_replace('_', ' ', $name)."</label>";
	           $sql2 = "select *  from academic_year where status='active'";
		       $result2 = mysqli_query($con, $sql2) or die("Error in Selecting " . mysqli_error($connection));
               echo "<select name='$name' class='form-control'>";
   ...
   elseif($i==8)
  {
	  echo "<div class='col-md-10'>
              <div class='form-group'>
			  <label>".str_replace('_', ' ', $name)."</label>";
	           $sql2 = "select *  from class where status='active'";
		       $result2 = mysqli_query($con, $sql2) or die("Error in Selecting " . mysqli_error($connection));
               echo "<select name='$name' class='form-control'>";
